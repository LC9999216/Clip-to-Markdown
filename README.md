...
| ChatGPT | 对话 | `chatgpt.com/c/*`、`chat.openai.com/c/*`，以及首页已有消息的临时对话 |
...
- 需要登录才能查看的内容会提示「需要登录」。
- ChatGPT 只保存用户与助手的文本 / Markdown 语义；文件附件、图片附件、Canvas、语音、Thinking 内部过程等暂不保存。登录页、设置页、空首页等无对话消息的页面不会被识别。
