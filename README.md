...
一键把 **X / Twitter、ChatGPT、B 站、知乎、小黑盒** 中的内容保存为干净 Markdown，并支持直接发送到 **Obsidian**。还提供「一图速览」：通过你配置的 AI 服务，把当前平台内容变成两句话摘要、核心观点和内容结构；可确定映射的结构条目还可定位回原文或视频时间点。
...
- **一图速览 V2**：对 X、知乎、小黑盒、ChatGPT 和 B 站内容生成两句话摘要、核心观点与内容结构；DOM 内容支持定位原文，B 站支持定位字幕/章节时间点
- **B站独立字幕页**：B 站视频/分 P 页面的侧栏右上角出现「字幕」入口，点击进入独立字幕页，阅读官方字幕轨并跟随播放位置
...
在受支持平台的具体内容页打开 Side Panel，按 `Ctrl + Shift + Y`，即可生成当前内容的两句话摘要、核心观点与内容结构。
...
- **来源定位**：知乎、小黑盒、ChatGPT 和 X Article 的结构条目会重新核对页面、来源块编号和短引用，只有唯一匹配时才滚动并高亮；B 站条目定位到对应章节或字幕时间点，不会自动播放
- **普通推文不伪装可跳转**：没有可靠原文锚点时，内容结构保持静态展示
- **看完再收藏**：Side Panel 里确认内容有价值后，点击「保存 Markdown」即可复用原有保存链路

> 一图速览只在当前页面主动触发时读取内容，并发送给用户配置的 AI 服务。B 站无字幕时仍可保存标题、简介和章节，并在分析中明确标注暂无字幕。

## 🎬 B站独立字幕页

在 B 站视频或分 P 页面打开侧栏时，右上角会出现「字幕」入口（仅 B 站视频页显示，与一图速览是否生成无关）。点击进入独立字幕页 `subtitle.html`；点击「← 返回一图速览」回到 `sidepanel.html`。

功能范围：

- **仅官方字幕**：只读取 B 站官方人工或 AI 字幕轨（`x/player/wbi/v2` 与字幕 CDN），无ASR、无第三方接口、无本地语音识别兜底；视频没有字幕轨时明确提示
- **轨道切换**：按「人工中文 → AI 中文 → 其他人工 → 其他 AI」自动选择，也可手动切换；轨道选择与滚动位置在当前浏览器会话内记忆
- **时间跳转**：点击字幕行跳转播放器到对应时间，保持原本的播放/暂停状态不变
- **当前句高亮与自动跟随**：页面可见时每 0.5 秒跟随播放位置；手动滚动后暂停跟随，点击「回到当前句」立即回到当前句并恢复跟随
- **会话缓存**：同一视频、同一分 P、同一轨道的首次加载结果在当前浏览器会话内复用；点击刷新强制重新获取视频信息、字幕轨与正文

字幕页不提供搜索、段数统计、复制、导出、翻译、双语对照、顺句或 ASR；需要登录 B 站才能查看字幕轨的视频会明确提示登录。
...
- **B 站**：打开 `/video/BV...` 视频详情页；只保存视频信息与可用字幕，不下载视频。字幕功能请使用侧栏右上角的「字幕」入口进入独立字幕页
...
- X 视频不会完整下载
- 知乎折叠长回答只保存已展开部分
- B 站没有可用字幕时不会自动语音识别；一图速览改用标题、简介和章节等可用元数据
- B站独立字幕页仅官方字幕（无ASR），不做翻译、双语、顺句；字幕轨需要登录的视频会提示先登录，播放器未加载时点击字幕行只提示稍后重试
- ChatGPT 的文件附件、图片附件、Canvas、语音和 Thinking 内部过程暂不作为独立内容保存
- Obsidian 模式需要本机运行 Local REST API
- 使用一图速览前需配置 OpenAI-Compatible AI 服务；B 站时间定位依赖当前页面播放器仍然可用
