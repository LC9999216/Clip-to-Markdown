...
一键把 **X / Twitter、ChatGPT、B 站、知乎、小黑盒** 中的内容保存为干净 Markdown，并支持直接发送到 **Obsidian**。还提供「一图速览」：通过你配置的 AI 服务，把 X 内容变成两句话摘要、核心观点和内容结构；X Article 的结构条目还可定位回原文段落。
...
- **一图速览 V2**：对 X 内容生成两句话摘要、核心观点与内容结构；X Article 支持从结构条目定位原文
...
在 X / Twitter 上打开具体内容页（`/status/{id}` 推文或 X Article），按 `Ctrl + Shift + Y` 打开 Side Panel，即可生成当前内容的两句话摘要、核心观点与内容结构。
...
- **X Article 可定位原文**：有原文锚点的结构条目可点击；扩展会重新核对文章、原文块编号和短引用，只有唯一匹配时才滚动并高亮对应段落
- **普通推文不伪装可跳转**：没有可靠原文锚点时，内容结构保持静态展示
- **看完再收藏**：Side Panel 里确认内容有价值后，点击「保存 Markdown」即可复用原有保存链路

> 一图速览 V2 仅支持 X / Twitter；原文定位仅支持 X Article。其他平台页面仍可正常使用原有 Markdown 保存功能。
...
- 一图速览 V2 仅支持 X / Twitter 内容，原文定位仅支持 X Article；使用前需配置 OpenAI-Compatible AI 服务
