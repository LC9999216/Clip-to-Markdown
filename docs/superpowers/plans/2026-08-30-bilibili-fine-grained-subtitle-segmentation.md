...
- [ ] 每条源字幕独立切分，没有跨行合并。
- [ ] 正常目标约 4 秒；当源行平均每个 code point 不超过 6 秒时，子段硬上限为 6 秒。
- [ ] 中文目标 24、硬上限 28 code points。
- [ ] 拉丁目标 56、硬上限 72 code points。
- [ ] 强标点、弱标点、空格、硬切优先级均有测试。
- [ ] 小数、版本号、URL 中的句点不会误切。
- [ ] 时间按累计 code point 比例分配。
- [ ] 每条源行首尾时间精确保留。
- [ ] 相邻同源子段时间连续、不重叠。
- [ ] 真实源行空档保持为空档。
- [ ] 没有空文字段、重复文字或丢字。
- [ ] 极稀疏源行保真例外有测试和文档。
- [ ] 扩展汉字按一个 code point 处理。
- [ ] 官方中文轨集成测试通过。
- [ ] AI 中文虚拟轨集成测试通过。
- [ ] 点击短字幕跳到其新起始时间。
- [ ] 播放高亮按短时间段切换。
- [ ] 翻译缓存/失败回退/刷新语义未改变。
- [ ] 未增加设置、协议、依赖、网络调用或 ASR。
- [ ] `npm test`、typecheck、build、diff check 全绿。
- [ ] README 与进度报告准确说明比例估时限制。
- [ ] 3 个既有未提交文件哈希未变且未进入提交。
- [ ] 没有 merge、push、PR 或修改 `main`。
