...
用于生成两句话摘要、核心观点和内容结构。
对于 X Article，发送的正文包含扩展生成的原文块编号；
AI 返回的内容结构可包含不超过 140 字的短原文引用，
仅用于用户点击结构条目时核对并定位当前页面中的对应段落。
AI 服务的 Endpoint、API Key 和模型名称保存在浏览器本地
（chrome.storage.local），只由扩展后台读取。
API Key 不会进入网页、扩展消息或日志。

一图速览结果及其中的短原文引用只保存在浏览器会话存储
（chrome.storage.session）中，不写入网页、不写入长期本地配置，
也不会随「保存 Markdown」写入导出的正文。
