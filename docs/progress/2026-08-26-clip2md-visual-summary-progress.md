...
- [ ] Chrome 手工验证（见下方记录，需在真实浏览器中执行）
- [x] 最终完整 diff、安全和代码质量审查

## 七、Chrome 手工验证记录

> 本开发环境无法启动真实 Chrome 浏览器，以下场景**未在本环境执行手工验证**，
> 需在 `dist/` 加载扩展后按清单逐项验证。代码层面已由 341 项自动测试覆盖。

| 场景 | 结果 |
|---|---|
| X 普通推文一图速览 | 未验证（需手工） |
| X Article 长文一图速览 | 未验证（需手工） |
| 非 X 页面（知乎/B 站） | 未验证（需手工） |
| 未配置 AI 时的错误提示 | 未验证（需手工） |
| 缓存命中 / 重新生成 | 未验证（需手工） |
| 保存 Markdown（下载/自定义文件夹） | 未验证（需手工） |
| 暗色模式与 300px 窄宽 | 未验证（需手工） |
